...
module.exports = {
    ...
    states: {
        ...
        initial: async (ctx, state, data) => {
            ...
            const authorizationUrl = `https://auth.atlassian.com/authorize?audience=api.atlassian.com&` +
        ...
        authCallback: async (ctx, state) => {
            ...
            if (ctx.query.code) {
                ...
                throw { 
                    status: 500, 
                    message: `Atlassian authentication failed: ${ctx.query.error_description || ctx.query.error || 'Unknown error'}`, 
                    state 
                };
            }
        }
