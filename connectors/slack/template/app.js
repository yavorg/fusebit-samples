...
async function registerCore(req, res, isDeregister){
    ...
    try {
        await storage.put(() => {
            ...
            let handlerRegistered = storage.appToHandler[app] &&
                storage.appToHandler[app].includes(handler);
            if((!isDeregister && handlerRegistered) ||
                (isDeregister && !handlerRegistered)){
                let e = new Error();
